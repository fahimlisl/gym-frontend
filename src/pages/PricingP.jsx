...
export default function PricingP() {
  ...
  const handleStartNow = (plan) => {
    ...
    setCouponDiscount(0);
    setCouponError("");
    // Reset avatar on each modal open
    setAvatarFile(null);
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      if (coupon.expiryDate && new Date(coupon.expiryDate) < new Date())
        throw new Error("This coupon has expired");
      if (coupon.minCartAmount > selectedPlan.finalPrice)
        throw new Error("Coupon is not valid for 1 month plan");
      if (coupon.usageLimit && coupon.usedCount >= coupon.usageLimit)
        throw new Error("This coupon has reached its usage limit");
    ...
    try {
      ...
      if (data.data === true) {
        setRegistrationStatus({
          status: "error",
          message:
            "User with this email or phone number already exists. Please use different credentials or log in.",
        });
        return false;
      }
      return true;
  ...
  const handlePaymentAndRegistration = async () => {
    ...
    await handlePayment({
      ...
      onSuccess: async (paymentResult) => {
        ...
        try {
          // ── Use FormData so the avatar file is sent as multipart ──
          const registerPayload = new FormData();
          registerPayload.append("username", formData.fullName);
          registerPayload.append("email", formData.email);
          registerPayload.append("phoneNumber", formData.phoneNumber);
          registerPayload.append("planId", selectedPlan._id);
          registerPayload.append("coupon", appliedCoupon?.code || "");
          registerPayload.append("admissionFee", ADMISSION_FEE);
          registerPayload.append("discountTypeOnAdFee", formData.discountTypeOnAdFee);
          registerPayload.append("discountOnAdFee", formData.discountOnAdFee);
          registerPayload.append("paymentStatus", "paid");
          registerPayload.append("paymentMethod", "razorpay");
          registerPayload.append("paymentId", paymentResult.paymentId);
          registerPayload.append("orderId", paymentResult.orderId);
          // Append the actual file — field name must match multer config on backend ("avatar")
          ...
          const response = await fetch(
            ...
            {
              method: "POST",
              // ⚠️ Do NOT set Content-Type manually — browser sets multipart boundary automatically
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-neutral-900 rounded-2xl border border-white/10 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="relative bg-gradient-to-br from-[#1a0000] to-neutral-900 px-8 py-6 border-b border-white/7 rounded-t-2xl overflow-hidden">
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-white/40 mt-1">{selectedPlan?.title}</p>
                </div>
                <button
                  onClick={() => setShowModal(false)}
            ...
            {/* Offer banners */}
            {offerDetails.filter((o) => o.category === billing && o.isActive).length > 0 && (
              <div className="px-8 pt-5 flex flex-col gap-2">
                {offerDetails
                  .filter((o) => o.category === billing && o.isActive)
                  .map((offer) => (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-white/40 text-xs truncate mt-0.5">{offer.description}</p>
                        </div>
                      </div>
                      {offer.coupon && (
                        <button
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-5">
                      ...
                      <div className="relative shrink-0">
                        <div className="w-20 h-20 rounded-full border-2 border-dashed border-white/20 overflow-hidden bg-white/[0.04] flex items-center justify-center">
                          {avatarPreview ? (
                            <img
                              src={avatarPreview}
                              alt="Avatar preview"
                            ...
                            <svg width="26" height="26" viewBox="0 0 24 24" fill="none"
                              stroke="currentColor" strokeWidth="1.5" className="text-white/20">
                        ...
                        {avatarPreview && (
                          <button
                            ...
                            disabled={loading || checkingUser}
                            className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-600 flex items-center justify-center text-white text-[10px] hover:bg-red-700 transition"
                          >
                            ✕
                          </button>
                        )}
                      ...
                      {/* Upload area */}
                      <label className={`flex-1 cursor-pointer group ${loading || checkingUser ? "pointer-events-none opacity-40" : ""}`}>
                        <div className="w-full px-4 py-4 bg-white/[0.03] border border-dashed border-white/15 rounded-xl flex flex-col items-center gap-1.5 hover:border-red-500/40 hover:bg-white/[0.05] transition">
                          <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" strokeWidth="2" className="text-white/30 group-hover:text-red-400 transition">
                          ...
                          <p className="text-xs text-white/30 group-hover:text-white/50 transition text-center">
                            {avatarFile
                              ? <span className="text-green-400 font-semibold">{avatarFile.name}</span>
                              : <><span className="text-white/60 font-semibold">Click to upload</span> or drag & drop</>
                            }
                          </p>
                    ...
                    {!appliedCoupon ? (
                      ...
                      <div className="flex items-center justify-between bg-green-500/10 border border-green-500/25 rounded-lg px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-white/40 text-xs">— ₹{couponDiscount} off</span>
                        </div>
                        <button
                          type="button"
                          onClick={removeCoupon}
