...
const schema = z.object({
  ...
  password: z.string().min(3, "Password must be at least 3 characters"), // will be changing the min to 6 or 8 later , before pushin to production 
});
          ...
          <div className="grid grid-cols-3 border border-white/10">
            {Object.keys(ROLE_MAP).map((key) => (
              <button
                ...
                onClick={() => setRole(key)}
                className={clsx(
                  "py-3 text-xs font-extrabold tracking-widest transition-all duration-200",
                  role === key
