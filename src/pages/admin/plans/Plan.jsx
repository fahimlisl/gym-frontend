...
import { Plus, Trash2, Pencil, Package, Check, X, Lock } from "lucide-react";
...
export default function AdminPlans() {
  ...
  const isAllowed = isSuperAdmin || !!admin?.plans?.allow;
  const isReadOnly = !isSuperAdmin && !!admin?.plans?.isReadOnly;
  ...
  if (!isAllowed) {
    return (
      <div className="p-6 text-white flex flex-col items-center justify-center h-[60vh] text-center">
        <Lock size={40} className="text-red-600 mb-4" />
        <h2 className="text-lg font-bold mb-1">Access restricted</h2>
        <p className="text-gray-400 text-sm">
          You don't have permission to view plan management.
        </p>
      </div>
    );
  }
      ...
      <div className="bg-neutral-900 border border-white/10 rounded-xl p-6 mb-10">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <input
            placeholder="Title"
            disabled={!canEdit}
            className="bg-black border border-white/10 p-2 rounded disabled:opacity-40 disabled:cursor-not-allowed"
            value={form.title}
