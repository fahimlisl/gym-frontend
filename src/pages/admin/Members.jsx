...
export default function Members() {
  ...
  const filteredUsers = useMemo(() => {
    let filtered = users.filter((u) => {
      ...
      switch (filter) {
        case "expired": matchesFilter = status === "expired"; break;
        case "active": matchesFilter = status === "active"; break;
        case "withPT": matchesFilter = hasPT === true; break;
      ...
      <div className="space-y-10">
        ...
        {ptAlerts.length > 0 && activeTab === "members" && search.trim().length === 0 && (
          <div className="rounded-xl border border-yellow-500/30 bg-yellow-500/5 p-4 space-y-2">
            ...
            <div className="space-y-1.5">
              {ptAlerts.map((alert) => (
                <div key={alert.id} className="flex items-center justify-between bg-yellow-500/[0.06] border border-yellow-500/20 rounded-lg px-3 py-2">
                  <div className="flex items-center gap-2">
                    <span className="text-white text-xs font-bold">{alert.name}</span>
                    <span className="text-yellow-400/70 text-[10px]">— {alert.missing.join(" & ")} not provided</span>
                  </div>
                  <Link to={`/admin/members/${alert.id}`} className="text-[10px] font-black tracking-widest text-yellow-400 hover:text-white border border-yellow-500/30 hover:bg-yellow-500/20 px-2.5 py-1 rounded-md transition">
                    ASSIGN →
                  </Link>
                </div>
              ))}
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <input
            ...
            className="flex-1 bg-black border border-white/10 px-4 py-3 text-sm text-white rounded-lg focus:outline-none focus:border-red-500 transition"
          />
          <select
            value={filter}
            onChange={(e) => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 sm:gap-6">
          ...
          <StatBox label="WITH PT" value={users.filter((u) => u.personalTraning?.subscription?.length > 0).length} />
          <StatBox label="WITHOUT PT" value={users.filter((u) => !u.personalTraning?.subscription?.length).length} />
          <StatBox label="ACTIVE" value={users.filter((u) => getLatestStatus(u) === "active").length} />
          <StatBox label="EXPIRED" value={users.filter((u) => getLatestStatus(u) === "expired").length} />
        </div>
...
function StatBox({ label, value }) {
  return (
    <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-black via-neutral-900 to-black border border-white/10 p-4 sm:p-5">
      <p className="text-[10px] sm:text-[11px] text-gray-400 tracking-widest">{label}</p>
      <p className="text-xl sm:text-3xl font-black mt-2 sm:mt-3">{value}</p>
