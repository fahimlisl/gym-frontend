...
export default function TrainerScanGymQR() {
  ...
  const handleCheckin = async () => {
    setStatus("loading");
    try {
      const res = await api.post("/trainer/attendance/trainer/gym-qr");
      if (res.data.isCheckout) {
        setStatus("checkout");
      } else {
        setStatus("success");
      }
      ...
      if (data?.alreadyCheckedOut) setStatus("already_out");
  ...
  const getCardStyle = () => {
    switch (status) {
      ...
      case "checkout":  return "border-blue-500/30 bg-blue-500/10";
      case "already_out": return "border-yellow-500/30 bg-yellow-500/10";
  ...
  const getTextStyle = () => {
    switch (status) {
      ...
      case "checkout":  return "text-blue-400";
      case "already_out": return "text-yellow-400";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-neutral-900 to-black flex flex-col items-center justify-center gap-6 p-4">
      ...
      {status === "idle" || status === "loading" ? (
        ...
        <div className={`w-72 rounded-2xl border p-6 flex flex-col items-center gap-4 ${getCardStyle()}`}>
          <span className="text-4xl">{getEmoji()}</span>
          <p className={`text-sm font-bold tracking-wide text-center ${getTextStyle()}`}>
            {message}
          </p>

          {/* only show scan again if not permanently done for the day */}
          {status !== "already_out" ? (
            <button
              onClick={reset}
            ...
            <p className="text-white/30 text-xs tracking-widest text-center">
              See you tomorrow 👊
            </p>
          )}
        </div>
      ...
      <p className="text-white/20 text-xs tracking-widest">Point camera at the gym's QR code</p>
    </div>
  );
}