...
export default function Cart({ isOpen, onClose, cart, onUpdateQuantity, onRemoveItem, total }) {
  ...
  const handleCheckout = async () => {
    ...
    await handlePayment({
      ...
      onError: (err) => {
        toast.error(err.message === "Payment modal closed by user"
          ? "Payment cancelled"
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-md bg-zinc-900 shadow-2xl">
            ...
            <div className="border-t border-zinc-800 p-6 space-y-4">
              ...
              <div className="space-y-3">
                <input type="tel" placeholder="PHONE NUMBER *"
                  value={customerInfo.phoneNumber} maxLength={10}
                ...
                {existingUser && !showFullForm && (
                  <div className="px-4 py-2 bg-green-900/20 border border-green-600/40 rounded-lg">
                    ...
                    {existingUser.email && <p className="text-gray-400 text-xs">{existingUser.email}</p>}
                  </div>
                )}
                {showFullForm && (
                  <>
                    <input type="text" placeholder="FULL NAME *" value={customerInfo.fullName}
                    ...
                    />
                    <input type="email" placeholder="EMAIL (OPTIONAL)" value={customerInfo.email}
