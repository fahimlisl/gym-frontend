...
export default function CafeAdminDashboard() {
  ...
  useEffect(() => {
    ...
    const filtered = items.filter((item) =>
      item.barcode?.toLowerCase().includes(barcodeQuery.toLowerCase())
    );
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Coupon cannot be applied");
    } finally {
      setCouponLoading(false);
    }
  };
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl shadow p-6">
          ...
          <div className="relative mb-6">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && filteredBarcodes.length > 0) {
                  handleAddByBarcode(filteredBarcodes[0].barcode);
                }
              }}
            />

            {showDropdown && filteredBarcodes.length > 0 && (
              <div className="absolute z-50 w-full bg-white border rounded-xl mt-1 max-h-60 overflow-y-auto shadow-lg">
                {filteredBarcodes.map((item) => (
                  <div
                    key={item._id}
                    onClick={() => handleAddByBarcode(item.barcode)}
        ...
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col">
          ...
          {!cart || cart.items.length === 0 ? (
            ...
            <>
              <div className="flex-1 space-y-3 overflow-y-auto">
                {cart.items.map((i) => (
                  <div key={i._id} className="flex justify-between items-center bg-gray-50 p-3 rounded-xl">
                    <div>
                      <p className="font-semibold">{i.name}</p>
                      <p className="text-sm text-gray-600">
                        ₹{i.price} × {i.quantity}
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => handleDecrementItem(i.item)} className="w-8 h-8 rounded-full border font-bold">−</button>
                      <span className="min-w-[80px] text-right font-bold">₹{i.price * i.quantity}</span>
                      <button onClick={() => handleIncrementItem(i.item)} className="w-8 h-8 rounded-full border font-bold">+</button>
                      <button onClick={() => handleRemoveItem(i.item)} className="text-red-600 text-sm ml-2">Remove</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Customer Details */}
              <div className="mt-3 space-y-2">
                <input className="w-full border rounded-xl px-4 py-3" placeholder="Customer name" value={name} onChange={(e) => setName(e.target.value)} />
                <input className="w-full border rounded-xl px-4 py-3" placeholder="Phone number" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} />
                <input className="w-full border rounded-xl px-4 py-3" placeholder="Email (optional)" value={email} onChange={(e) => setEmail(e.target.value)} />
              </div>

              {/* Payment */}
              <div className="mt-6 space-y-4">
                <select className="w-full border rounded-xl px-4 py-3" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}>
                  <option value="cash">Cash</option>
                  <option value="upi">UPI</option>
                  <option value="card">Card</option>
                </select>
                ...
                {paymentMethod === "upi" && (
                  <input className="w-full border rounded-xl px-4 py-3" placeholder="UPI Reference ID" value={upiRef} onChange={(e) => setUpiRef(e.target.value)} />
                )}
