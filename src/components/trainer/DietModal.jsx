...
export default function DietModal({ studentId, onClose }) {
    ...
    const init = async () => {
      try {
        ...
        if (check.data.data.exists) {
          ...
          // check if any macro gram is actually set (> 0)
          const hasMacros =
            dietData.desiredMacros &&
  ...
  const macroProgress = targetCalories
    ? (macroCalories / targetCalories) * 100
    : 0;
  ...
  const adjustMacro = (type, direction) => {
    if (isApproved) {
      toast.error("Cannot edit approved diet");
      return;
    }
    const amount = direction === "up" ? 1 : -1;
    const setter = {
      protein: setMacroProtein,
      carbs: setMacroCarbs,
      fats: setMacroFats,
    }[type];
  ...
  const presets = [
    { name: "BAL", p: 0.3, c: 0.4, f: 0.3, icon: "⚖️", mobile: "⚖️" },
    { name: "PRO", p: 0.4, c: 0.35, f: 0.25, icon: "💪", mobile: "💪" },
    { name: "LC", p: 0.35, c: 0.2, f: 0.45, icon: "🥑", mobile: "🥑" },
    { name: "KETO", p: 0.25, c: 0.05, f: 0.7, icon: "🥩", mobile: "🔥" },
    { name: "END", p: 0.2, c: 0.6, f: 0.2, icon: "🏃", mobile: "⚡" },
  ];
  ...
  const saveMacros = async () => {
    ...
    if (Math.abs(macroDifference) > 50) {
      toast.error("Macros must be within 50kcal of target");
      return;
    }
  ...
  // macros things starts here

  // build a quick lookup map for foods
  ...
  // calculate macros from already added foods
  ...
  // calculate macros from selected foods (pending addition)
  ...
  const addFoodsToDiet = async () => {
    const foodsArray = Object.values(selectedFoods).map(
      ({ foodId, grams }) => ({
        foodId,
        grams,
      }),
  ...
  const removeFood = async (foodId) => {
    try {
      const res = await api.patch(
        `/trainer/diet/${studentId}/food/remove/${foodId}`,
        {
          data: { userId: studentId }, // DELETE needs body inside data
        },
      );
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-md flex items-end md:items-center justify-center animate-in fade-in duration-300">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-6 space-y-6">
          {step === "create" && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="relative">
                  <label className="block text-[10px] tracking-wider text-gray-500 mb-1 ml-1">
                    GOAL
                  </label>
                  <select
                    value={goal}
                  ...
                  <div className="absolute right-4 bottom-4 text-gray-500 pointer-events-none text-xs">
                    ▼
                  </div>
                </div>
          ...
          {step === "macros" && diet && (
            <div className="space-y-5 sm:space-y-6">
              <div className="bg-gradient-to-r from-yellow-500/10 to-amber-500/10 border border-yellow-500/30 rounded-2xl p-3 sm:p-4">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[9px] sm:text-[10px] tracking-wider text-gray-500 truncate">
                      DIET STATUS
                    </p>
                    <p className="text-xs sm:text-sm font-black text-yellow-400 truncate">
                      DRAFT · SET MACROS
                    </p>
                  </div>
              ...
              <div className="flex justify-center py-1 sm:py-2">
                <div className="relative w-28 h-28 sm:w-32 sm:h-32">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-[7px] sm:text-[8px] tracking-wider text-gray-500/80">
                      OF {diet.calories}
                    </span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                {[
                  ...
                  },
                  {
                    ...
                    icon: "🥑",
                    activeGlow: "shadow-green-400/20",
                  },
                ].map((macro) => (
                  <div
                    key={macro.label}
                    className={`
                      ...
                      border ${activeMacro === macro.type ? macro.border : "border-white/5"} 
                      ${macro.bg} backdrop-blur-sm 
                      p-3 sm:p-4
                      transition-all duration-200 ease-out
                      active:scale-[0.98]
                      ${activeMacro === macro.type ? `shadow-lg ${macro.activeGlow}` : ""}
                  ...
                  >
                    <div className="flex items-center justify-between w-full mb-1">
                      ...
                      <span
                        className={`text-[8px] sm:text-[9px] tracking-wider ${macro.color} font-bold`}
                      >
                        {macro.label}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-col items-center w-full">
                      <button
                        onClick={() => adjustMacro(macro.type, "up")}
                        className="
                          ...
                          active:scale-90 active:bg-red-500/20 active:border-red-500
                      ...
                      <div className="relative my-0.5 sm:my-1">
                        <span
                          className={`text-3xl sm:text-4xl font-black ${macro.color} tabular-nums leading-none`}
                        >
                          {macro.value}
                        </span>
                        <span className="text-[9px] sm:text-[10px] text-gray-500 ml-0.5 font-medium">
                          g
                        </span>
                      </div>

                      <button
                        onClick={() => adjustMacro(macro.type, "down")}
                        className="
                          w-8 h-8 sm:w-10 sm:h-10
                          rounded-full
                          bg-black/40
                    ...
                    <div className="text-[9px] sm:text-[10px] text-gray-500/80 mt-2">
                      {macro.kcal} kcal
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <span className="text-[9px] sm:text-[10px] tracking-wider text-gray-500/80">
                    PRESETS
                  </span>
                ...
                <div className="flex gap-1.5 overflow-x-auto pb-1 no-scrollbar">
                  {presets.map((preset) => (
                    <button
                      ...
                      className="flex-shrink-0 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl bg-white/5 border border-white/10 transition-all duration-200 ease-out active:scale-95 active:bg-red-500/20 active:border-red-500/50 hover:bg-white/10"
                    >
                      <span className="text-sm sm:text-base mr-1">
                        {preset.mobile}
                      </span>
              ...
              <button
                ...
                className="
                  ...
                  font-black tracking-wider text-sm sm:text-base 
                  bg-gradient-to-r from-red-600 to-red-500 text-white 
                  shadow-lg shadow-red-500/30 
                  transition-all duration-300 ease-out
                  active:scale-[0.98] 
                  disabled:opacity-50 disabled:from-gray-600 disabled:to-gray-500 
              ...
              >
                ...
                {savingMacros ? (
                  ...
                  <span className="flex items-center justify-center gap-2">
                    <span>🔒</span>
                    {Math.abs(macroDifference) > 50
                      ? "BALANCE MACROS FIRST"
                      : "SAVE & CONTINUE"}
                  </span>
                )}
              </button>
          ...
          {step === "food-insertion" && diet && (
            <div className="space-y-6">
              ...
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[10px] font-mono text-white">
                    {macroProtein}P / {macroCarbs}C / {macroFats}F
                  </span>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <div className="space-y-1">
                      <div className="flex justify-between text-[8px]">
                        ...
                        <span className="text-gray-400">
                          {Math.round(totalMacros.protein)} / {macroProtein}g
                        </span>
                      </div>
                      <div className="w-full h-1.5 bg-neutral-900 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min((totalMacros.protein / macroProtein) * 100, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-[8px]">
                        ...
                        <span className="text-gray-400">
                          {Math.round(totalMacros.carbs)} / {macroCarbs}g
                        </span>
                      </div>
                      <div className="w-full h-1.5 bg-neutral-900 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min((totalMacros.carbs / macroCarbs) * 100, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-[8px]">
                        <span className="text-green-400">Fats</span>
                        <span className="text-gray-400">
                          {Math.round(totalMacros.fats)} / {macroFats}g
                        </span>
                      </div>
                      <div className="w-full h-1.5 bg-neutral-900 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min((totalMacros.fats / macroFats) * 100, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <div
                    className={`flex justify-between items-center p-2 rounded-lg ${
                      totalMacros.calories === macroCalories
                  ...
                  >
                    ...
                    >
                      {macroCalories - Math.round(totalMacros.calories)} kcal
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-[10px] tracking-wider text-gray-500 ml-1">
                    SEARCH FOODS
                  </label>
                ...
                {Object.keys(selectedFoods).length > 0 && (
                  <div className="mt-2 p-3 rounded-xl bg-green-500/5 border border-green-500/30 backdrop-blur-sm">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-[10px] tracking-wider text-green-400">
                        SELECTED ITEMS
                      </span>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {Object.values(selectedFoods).map((item) => (
                        <div
                          key={item.foodId}
                          className="group flex items-center gap-2 bg-black/60 border border-green-500/30 rounded-full pl-3 pr-1 py-1 hover:bg-green-500/10 transition-all"
                        >
                          ...
                          <button
                            ...
                            className="w-5 h-5 rounded-full bg-red-500/20 border border-red-500/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                          >
                ...
                {searchResults.length > 0 && (
                  <div className="space-y-3 mt-4">
                    ...
                    <div className="space-y-3">
                      {searchResults.map((food) => {
                        ...
                        const calories = Math.round(
                          (food.nutrition?.calories || 0) * scale,
                        );
                        const protein = (
                          (food.nutrition?.protein_g || 0) * scale
                        ).toFixed(1);
                        const carbs = (
                          (food.nutrition?.carbs_g || 0) * scale
                        ).toFixed(1);
                        const fats = (
                          (food.nutrition?.fat_g || 0) * scale
                        ).toFixed(1);
                    ...
                    ${
                      isSelected
                        ? "border-green-500 bg-gradient-to-br from-green-500/20 to-green-500/5 shadow-[0_0_30px_rgba(34,197,94,0.2)]"
                  ...
                  `}
                          >
                              ...
                              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-1">
                                    ...
                                    {isSelected && (
                                      <span className="text-[10px] font-bold bg-green-500 text-black px-2 py-0.5 rounded-full">
                                        SELECTED
                                      </span>
                                    )}
                                  </div>
                                  <p className="text-[11px] text-gray-400">
                                    {calories} kcal · P {protein}g · C {carbs}g
                                    · F {fats}g
                                  </p>
                                  <p className="text-[9px] text-gray-600 mt-1">
                                    per {grams}g ({baseQty}g base)
                                  </p>
                                </div>
                            ...
                            ${
                              isSelected
                                ? "bg-green-600 text-white shadow-[0_0_20px_rgba(34,197,94,0.5)] hover:bg-green-500"
                    ...
                    {Object.keys(selectedFoods).length > 0 && (
                      <button
                        ...
                        disabled={addingFood}
                        className="
                relative w-full mt-4 py-4 rounded-xl font-black tracking-wider text-sm
                bg-gradient-to-r from-green-600 to-emerald-600 text-white
                shadow-lg shadow-green-500/30
                transition-all duration-300 ease-out
                active:scale-[0.98] disabled:opacity-50 disabled:from-gray-600 disabled:to-gray-500
                overflow-hidden group
              "
                      >
                        <span className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
                        {addingFood ? (
                          ...
                          <span className="flex items-center justify-center gap-2">
                            <span>➕</span>
                            ADD {Object.keys(selectedFoods).length} SELECTED
                            FOODS
                          </span>
                        )}
                      </button>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-gray-400 bg-white/5 px-2 py-1 rounded-full">
                      {diet.foods.length}{" "}
                      {diet.foods.length === 1 ? "item" : "items"}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2">
                    {diet.foods.map((food) => (
                      <div
                        key={food._id}
                        className="group flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 hover:border-red-500/30 transition-all"
                      >
                        <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            className="w-8 h-8 rounded-full bg-red-600/20 border border-red-600/30 text-red-400 text-sm flex items-center justify-center opacity-70 group-hover:opacity-100 hover:bg-red-600/30 hover:scale-110 transition-all"
                          >
                            ✕
                          </button>
                        </div>
              ...
              <button
                ...
                disabled={approving}
                className="
        relative w-full py-5 rounded-xl font-black tracking-wider text-base
      ...
      "
          ...
          {step === "approved" && diet && (
            <div className="space-y-6">
              <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/40 rounded-2xl p-5">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-green-500/30 flex items-center justify-center">
                    <span className="text-green-400 text-2xl">✅</span>
                  </div>
                  <div>
                    <p className="text-xs tracking-wider text-green-400/80">
                      APPROVED DIET
                    </p>
              ...
              <div className="bg-white/5 rounded-xl p-5 border border-white/10">
                <p className="text-[10px] tracking-wider text-gray-500 mb-4">
                  DAILY TARGETS
                </p>
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    ...
                    <p className="text-xl font-black text-red-500">
                      {diet.desiredMacros?.protein?.grams || 0}g
                    </p>
                  ...
                  <div>
                    ...
                    <p className="text-xl font-black text-yellow-500">
                      {diet.desiredMacros?.carbs?.grams || 0}g
                    </p>
                  ...
                  <div>
                    ...
                    <p className="text-xl font-black text-green-500">
                      {diet.desiredMacros?.fats?.grams || 0}g
                    </p>
                    <p className="text-[8px] text-gray-500">FATS</p>
                  </div>
                </div>
              </div>
