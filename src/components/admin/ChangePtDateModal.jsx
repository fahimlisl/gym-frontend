...
export default function ChangePtDateModal({ userId, currentStart, currentEnd, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      await api.patch(`/admin/ptbill/change/date/${userId}`, {
        startDate: toBackendFormat(startDate),
        endDate: toBackendFormat(endDate),
      });
      toast.success("PT bill dates updated successfully");
      onSuccess?.(startDate, endDate); // now passes back the new dates
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md border border-red-600/40 bg-neutral-950 rounded-xl overflow-hidden shadow-[0_0_60px_rgba(220,38,38,0.15)]">
        <div className="border-b border-red-600/20 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="font-black tracking-widest text-sm">EDIT PT BILL DATES</h2>
            <p className="text-xs text-gray-500 mt-0.5">Modifies latest PT subscription &amp; transaction record</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 space-y-5">
          <div className="flex items-start gap-3 bg-red-600/10 border border-red-600/30 rounded-lg px-4 py-3">
            ...
            <p className="text-xs text-red-400 leading-relaxed">
              This will update both the PT bill validity and the linked transaction date. Use carefully.
            </p>
          </div>
