...
export default function AdminDietModal({ userId, onClose }) {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const check = await api.get(`/admin/diet/check/${userId}`);
        if (check.data.data.exists) {
          const dietRes = await api.get(`/admin/diet/show/${userId}`);
  ...
  const fetchAllFoods = async () => {
    try {
      const res = await api.get("/admin/getAllFoods");
  ...
  const saveCalories = async () => {
    ...
    try {
      ...
      await api.patch(`/admin/diet/edit/calories/${diet._id}`, { calories: newCal });
  ...
  const createDiet = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.post("/admin/diet/generate", {
        userId,
        goal,
        dietType,
  ...
  const saveMacros = async () => {
    ...
    try {
      ...
      const res = await api.patch(`/admin/diet/setMacros/${diet._id}`, {
  ...
  const createNewMeal = async () => {
    ...
    try {
      ...
      const res = await api.patch(`/admin/diet/add/meal/${diet._id}`, { meal: newMealName });
      setDiet(res.data.data);
  ...
  const removeMeal = async (mealId) => {
    ...
    try {
      const res = await api.patch(`/admin/diet/remove/meal/${mealId}/${diet._id}`);
      setDiet(res.data.data);
      if (activeMeal === mealId) {
        const remaining = res.data.data.meals || [];
        setActiveMeal(remaining.length > 0 ? remaining[0]._id : null);
      }
  ...
  const addFoodsToDiet = async () => {
    ...
    if (!foodsArray.length) return;
    try {
      setAddingFood(true);
      const res = await api.post(`/admin/addFood/${activeMeal}`, {
        userId,
        foods: foodsArray,
      });
      setDiet(res.data.data);
  ...
  const removeFood = async (foodId, mealId) => {
    try {
      const res = await api.patch(`/admin/diet/${userId}/food/remove/${foodId}/${mealId}`);
      setDiet(res.data.data);
  ...
  const approveDiet = async () => {
    try {
      setApproving(true);
      const res = await api.patch(`/admin/diet/approve/${diet._id}`);
      setDiet(res.data.data);
      setStep("approved");
  ...
  const editMacros = async () => {
    try {
      const res = await api.patch(`/admin/diet/approve/${diet._id}`, { status: "draft" });
      setDiet(res.data.data);
      setStep("macros");
            ...
            <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[3, 4, 5, 6].map((num) => (
                      <button
                        ...
                        className={`py-3 rounded-xl border text-sm font-bold transition-all ${mealsPerDay === num
              ...
              >
                {loading ? (
                  ...
                  <span className="flex items-center justify-center gap-2"><span>🚀</span> GENERATE DIET</span>
                )}
              </button>
            </div>
          )}
            ...
            <div className="space-y-5 sm:space-y-6">
              ...
              <div className="relative rounded-2xl border border-orange-500/30 bg-gradient-to-br from-orange-500/8 to-amber-500/5 backdrop-blur-sm p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {!editingCalories && (
                    <button
                      onClick={() => { setTempCalories(String(diet.calories)); setEditingCalories(true); }}
              ...
              <div className={`flex items-center justify-between p-3 sm:p-4 rounded-xl sm:rounded-2xl border transition-all duration-300 ${macroDifference === 0 ? "bg-green-500/10 border-green-500/30" :
                ...
                <span className={`text-sm sm:text-base font-black tabular-nums ${macroDifference === 0 ? "text-green-400" :
              ...
              <div className="bg-[#1C1C1E] rounded-2xl p-4 border border-white/5 shadow-xl">
                    ...
                    return (
                      <div key={macro.label} className={`flex-1 bg-gradient-to-br ${macro.gradient} rounded-xl p-3 flex flex-col items-center border border-white/10 backdrop-blur-sm`}>
                        ...
                        <div className="flex items-center gap-1 mt-2">
                          ...
                          <div className={`w-2 h-2 rounded-full ${isOver ? "bg-red-400 shadow-[0_0_6px_rgba(248,113,113,0.6)]" :
                ...
                <div className="flex items-center justify-between pt-3 border-t border-white/5">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <div className={`text-[10px] font-medium px-3 py-1 rounded-full bg-gradient-to-r ${totalMacros.calories === macroCalories ? "from-green-500/30 to-green-400/20 text-green-300 border border-green-500/30" :
                  ...
                  {diet.meals?.map((meal) => (
                    <div key={meal._id} className="flex-shrink-0 flex items-center gap-1 bg-white/5 rounded-full border border-white/10 pr-1">
                      <button
                        ...
                        className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${activeMeal === meal._id ? "bg-blue-500/20 text-blue-400" : "text-gray-400 hover:text-white"}`}
                      >
                        {meal.meal}
                      </button>
                      {diet.meals.length > 1 && (
                        <button
                          onClick={() => { if (window.confirm(`Delete "${meal.meal}"?`)) removeMeal(meal._id); }}
              ...
              {activeMeal ? (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={searchFoods}
                      className="px-5 py-3 rounded-xl bg-red-600/20 border border-red-600/30 text-red-400 font-bold text-sm active:scale-95 transition-all flex items-center gap-2"
                    >
                      <span>🔍</span>
                    ...
                    <div className="space-y-3 mt-4">
                      ...
                      <div className="space-y-3">
                        {searchResults.map((food) => {
                          ...
                          return (
                            <div key={food._id} className={`group relative overflow-hidden rounded-2xl border transition-all duration-300 ${isSelected
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <span className="flex items-center justify-center gap-2"><span>📊</span> EDIT MACROS</span>
                </button>
                ...
                >
                  ...
                  <span className="flex items-center justify-center gap-2"><span>🍽️</span> EDIT MEALS</span>
                </button>
              </div>
            </div>
          )}
