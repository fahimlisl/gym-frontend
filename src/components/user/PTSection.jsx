...
import { Dumbbell, Calendar, CreditCard, Award, Shield, RotateCcw } from "lucide-react";

// const IS_TESTING = process.env.NODE_ENV === "testing";
const IS_TESTING = process.env.NODE_ENV === "production";

export default function PTSection({ pt, subscription }) {
  ...
  if (loading) return <PTSkeleton />;
  if (status) return <PTRequestStatus status={status} />;

  if (pt && pt.subscription?.length && pt.subscription[pt.subscription.length - 1].trainer) {
  const current = pt.subscription.at(-1);
  const latestSub = subscription?.subscription?.[subscription.subscription.length - 1] ?? null;
  return <ActivePTSubscription subscription={current} userSubscription={latestSub} />;
}

  return <GetTrainerUI subscription={subscription}/>;
}


function ActivePTSubscription({ subscription, userSubscription }) {
  const trainer = subscription.trainer;

  const isPTExpired = subscription.status?.toLowerCase() === "expired"; // if  expired -> true

  const isSubActive = userSubscription?.status?.toLowerCase() === "active";  // if not active -> false
  const showRenewalBtn = IS_TESTING || (isPTExpired && isSubActive);
  ...
  return (
    <div className="relative group">
      ...
      <div className="relative bg-gradient-to-br from-neutral-900 via-black to-neutral-900
                    border border-red-600/30 rounded-xl overflow-hidden shadow-xl">
        ...
        <div className="p-6">
          ...
          <div className="bg-neutral-800/30 rounded-xl p-4 border border-neutral-700/50 mb-6">
...
function PTSkeleton() {
  return (
    <div className="border border-neutral-800 rounded-xl p-6
                  bg-gradient-to-br from-neutral-900 to-black animate-pulse">
